			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->padapter_link, ptdls_sta->phl_sta->mac_addr, 1, 3, PS_ANNC_DRV_RETRY_INT_MS);
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	if (ptdls_sta)
		issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->padapter_link, ptdls_sta->phl_sta->mac_addr, 0, 3, PS_ANNC_DRV_RETRY_INT_MS);
